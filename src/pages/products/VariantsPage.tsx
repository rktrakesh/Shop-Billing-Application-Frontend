...
import {
  Badge, Card, CardContent, Dialog, DialogContent, DialogHeader, DialogTitle, DialogBody, DialogFooter,
...
export default function VariantsPage() {
  ...
  const openCreate = () => { setEditing(null); reset({ stock: 0, minimumStock: 5 }); setFormOpen(true) }
  const openEdit = (v: ProductVariantResponse) => {
    setEditing(v)
    reset({
      productId: v.productId, productCode: v.productCode, color: v.color, size: v.size,
      sellingPrice: v.sellingPrice, costPrice: v.costPrice ?? 0, stock: v.stock, minimumStock: v.minimumStock,
      imageUrl: v.imageUrl,
    })
    setFormOpen(true)
  }
  ...
  const saveMutation = useMutation({
    mutationFn: (data: FormData) =>
      editing ? variantService.update(editing.id, data as ProductVariantRequest) : variantService.create(data as ProductVariantRequest),
    onSuccess: () => {
            ...
            <DialogBody className="space-y-3">
              <Controller
                ...
                render={({ field }) => (
                  ...
                  >
                    {products.map((p) => (
                      <SelectItem key={p.id} value={String(p.id)}>{p.designName}</SelectItem>
                    ))}
                  </SelectField>
                )}
              />
