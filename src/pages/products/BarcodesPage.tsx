...
export default function BarcodesPage() {
  ...
  const filtered = variants.filter((v) =>
    !search ||
    v.designName.toLowerCase().includes(search.toLowerCase()) ||
                  ...
                  {filtered.map((v) => (
                    <button
                      ...
                      className={`w-full flex items-center justify-between p-3 rounded-lg border transition-colors text-left ${
                        selected?.id === v.id ? 'border-primary/60 bg-primary/10' : 'border-border/30 hover:border-primary/30 hover:bg-card/30'
                      }`}
                    >
                      <div>
                        <p className="text-sm font-medium text-text-primary">{v.designName}</p>
                        <p className="text-xs text-text-muted">{v.color} · {v.size} · {v.productCode}</p>
                      </div>
                      <div className="text-right flex items-center gap-2">
                        {v.barcode ? (
                          <span className="font-mono text-xs text-primary">{v.barcode}</span>
                        ) : (
                          <Badge variant="muted">No barcode</Badge>
                        )}
                      </div>
                    </button>
                  ))}
              ...
              {!selected ? (
                ...
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div className="w-full bg-white text-black rounded-lg p-4 text-center border-2 border-dashed border-border/40">
                    ...
                    <p className="text-xs text-gray-600">{selected.color} | {selected.size}</p>
