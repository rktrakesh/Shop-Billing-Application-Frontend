      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-sidebar border border-border/40 rounded-2xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <Input label="Username" placeholder="Enter your username" error={errors.username?.message} {...register("username")} autoComplete="username" autoFocus />
            ...
            <Input
              ...
              rightIcon={
                <button type="button" onClick={() => setShowPassword((s) => !s)} className="text-text-muted hover:text-text-secondary transition-colors" tabIndex={-1}>
            ...
            <div className="flex items-center gap-2">
              <input type="checkbox" id="rememberMe" className="rounded border-border bg-card text-primary focus:ring-primary/30" {...register("rememberMe")} />
        ...
        <p className="text-center text-xs text-text-muted mt-4">© {new Date().getFullYear()} RKT Apparels. All rights reserved.</p>
      </div>
    </div>
  );
}
