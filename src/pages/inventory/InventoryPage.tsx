...
import { variantService, inventoryService } from "@/services";
...
const schema = z.object({
  ...
  changeType: z.enum(["ADD_STOCK", "SALE", "RETURN", "MANUAL_ADJUSTMENT"]),
  quantity: z.coerce.number().min(1, "Must be at least 1"),
  reason: z.string().optional(),
});
type FormData = z.infer<typeof schema>;

const CHANGE_TYPES: StockChangeType[] = ["ADD_STOCK", "SALE", "RETURN", "MANUAL_ADJUSTMENT"];
...
export default function InventoryPage() {
  ...
  const allVariants = allVariantsRes?.data?.data ?? [];
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-4">
        <button onClick={() => setTab("all")} className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${tab === "all" ? "bg-primary text-white" : "bg-card text-text-secondary hover:bg-card-hover"}`}>
          All Stock
        </button>
        <button onClick={() => setTab("low")} className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors flex items-center gap-1.5 ${tab === "low" ? "bg-danger text-white" : "bg-card text-text-secondary hover:bg-card-hover"}`}>
          ...
          <form onSubmit={handleSubmit((d) => adjustMutation.mutate(d))}>
            <DialogBody className="space-y-3">
              <Controller
                ...
                render={({ field }) => (
                  <SelectField label="Product Variant *" placeholder="Select variant" value={field.value ? String(field.value) : undefined} onValueChange={(v) => field.onChange(Number(v))} error={errors.productVariantId?.message}>
                    {allVariants.map((v) => (
              ...
              {(() => {
                ...
                if (changeType === "MANUAL_ADJUSTMENT") {
                  return <Input label="Set Stock To *" type="number" min={0} hint="Enter the correct total stock count — this replaces the current value, it does not add or subtract." error={errors.quantity?.message} {...register("quantity")} />;
                }
                const label = changeType === "SALE" ? "Quantity to subtract *" : "Quantity to add *";
                return <Input label={label} type="number" min={1} error={errors.quantity?.message} {...register("quantity")} />;
              })()}
              <Input label="Reason" placeholder="e.g. New stock arrival, stock count correction" {...register("reason")} />
            </DialogBody>
            <DialogFooter>
                ...
                {movements.map((m) => (
                  <div key={m.id} className="flex items-center justify-between p-3 rounded-lg border border-border/30">
                    <div className="flex items-center gap-3">
                      {["ADD", "PURCHASE", "RETURN"].includes(m.changeType) ? <ArrowUpCircle className="h-5 w-5 text-success" /> : <ArrowDownCircle className="h-5 w-5 text-danger" />}
                      <div>
                        <p className="text-sm font-medium text-text-primary">{getStockChangeLabel(m.changeType)}</p>
                        <p className="text-xs text-text-muted">
                          {m.reason || "No reason given"} · by {m.createdBy}
                        </p>
                      </div>
                    </div>
